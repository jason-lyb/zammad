class App.CTI extends App.Controller
  ...
  openExternalSearch: (e) ->
    ...
    # 전화번호 데이터 가져오기
    phoneNumber = $(e.currentTarget).data('phone')
    phoneClean = $(e.currentTarget).data('phone-clean')
    
    # 외부 사이트 URL 구성 (예시 - 실제 사용할 사이트에 맞게 수정)
    # 여러 외부 사이트 옵션을 제공할 수 있습니다
    
    # 옵션 1: 단일 외부 사이트 호출
    externalUrl = @buildExternalSearchUrl(phoneClean)
    ...
    window.open(
      externalUrl, 
      'externalSearchPopup', 
      "width=#{popupWidth},height=#{popupHeight},top=#{popupTop},left=#{popupLeft},resizable=yes,scrollbars=yes,noopener,noreferrer"
    )

  # 외부 검색 URL 생성
  buildExternalSearchUrl: (phoneNumber) ->
    # 설정에서 외부 사이트 URL 가져오기 (실제 구현 시 설정 파일에서 관리)
    baseUrl = 'https://backoffice-admin-space.callmaner.com/cid'
    
    # URL 파라미터로 전화번호 추가
    searchUrl = "#{baseUrl}?custTel=#{encodeURIComponent(phoneNumber)}"
    
    # 추가 파라미터가 필요한 경우
    # searchUrl += "&source=zammad&timestamp=#{new Date().getTime()}"
